...
const VideoDetails = () => {
  ...
  if (currentSubSectionIndex === -1) return true;

  const currentSection = courseSectionData[currentSectionIndex];

  const isLastSubInCurrentSection =
    currentSubSectionIndex === currentSection.subSection.length - 1;

  const isLastSection = currentSectionIndex === courseSectionData.length - 1;

  // If last subsection of last section, then it's truly last
  return isLastSubInCurrentSection && isLastSection;
};
  ...
  if (currentSubSectionIndex < noOfSubsections - 1) {
    // Next sub-section in the same section
    ...
    // Move to the next section's first sub-section
    const nextSectionId = courseSectionData[currentSectionIndex + 1]._id;
    const nextSubSectionId = courseSectionData[currentSectionIndex + 1].subSection?.[0]?._id;

    // Guard if next section has no sub-sections
    if (nextSubSectionId) {
      navigate(
  ...
  // Case 1: Previous subsection exists in the same section
  if (currentSubSectionIndex > 0) {
    const prevSubSectionId = courseSectionData[currentSectionIndex].subSection[currentSubSectionIndex - 1]._id;
    navigate(`/view-course/${courseId}/section/${sectionId}/sub-section/${prevSubSectionId}`);
  }
  // Case 2: Move to the last subsection of the previous section
  else if (currentSectionIndex > 0) {
    const prevSection = courseSectionData[currentSectionIndex - 1];
    const prevSectionId = prevSection._id;
    const lastSubSectionIndex = prevSection.subSection.length - 1;

    if (lastSubSectionIndex >= 0) {
      const prevSubSectionId = prevSection.subSection[lastSubSectionIndex]._id;
  ...
  const handleLectureCompletion = async () => {
    setLoading(true)
    console.log("Sending progress update", { courseId, subSectionId });
