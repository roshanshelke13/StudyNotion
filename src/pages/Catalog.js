...
import { catalogPage } from '../services/operations/CatalogAPI';
import { useSelector } from 'react-redux';
import Error from "./Error"
import Footer from "../components/common/Footer"
import CourseSlider from '../components/core/CatalogPage/CourseSlider';
import Courses from '../components/core/CatalogPage/Courses';
...
const Catalog = () => {
    ...
    useEffect (() => {
        if(categoryId){            
            (async () => {
                const response = await catalogPage(categoryId);
    ...
    console.log("data",catalogPageData);
    ...
    <>
        ...
        <div className='mx-auto box-content w-full max-w-maxContentTab px-4 py-12 lg:max-w-maxContent'>
            ...
            {catalogPageData?.data?.mostSellingCourses
              ?.slice(0, 4)
              .map((course, i) => (
                <Courses course={course} key={i} Height={"h-[400px]"} />
              ))}
          </div>
            </div>
        </div>
